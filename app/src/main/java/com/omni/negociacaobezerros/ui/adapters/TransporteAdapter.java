...
import static com.omni.negociacaobezerros.helpers.ViewHelper.setPluralText;
...
public class TransporteAdapter extends ListAdapter<TransporteUiState, TransporteAdapter.ViewHolder> {
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        void bind(TransporteUiState estado, boolean isSingleItem) {
            Context context = itemView.getContext();
            setText(binding.textoTipoVeiculo, estado.getNomeVeiculo());
            setPluralText(binding.textoQuantidadeVeiculos, context, R.plurals.plural_quantidade_veiculos, estado.getQuantidadeVeiculos());
            setText(binding.textoCapacidadeCabecas, context, R.string.formato_capacidade_cabecas, estado.getCapacidadeCabecas());
            setText(binding.textoPorcentagemOcupada, context, R.string.formato_numero_percentual, estado.getPorcentagemOcupada());
    ...
    private static class DiffCallback extends DiffUtil.ItemCallback<TransporteUiState> {
        ...
        public boolean areItemsTheSame(@NonNull TransporteUiState oldItem, @NonNull TransporteUiState newItem) {
            return Objects.equals(oldItem.getNomeVeiculo(), newItem.getNomeVeiculo());
        }

        @Override
        public boolean areContentsTheSame(@NonNull TransporteUiState oldItem, @NonNull TransporteUiState newItem) {
            return Objects.equals(oldItem.getNomeVeiculo(), newItem.getNomeVeiculo())
                    && oldItem.getQuantidadeVeiculos() == newItem.getQuantidadeVeiculos()
                    && oldItem.getCapacidadeCabecas() == newItem.getCapacidadeCabecas()
                    && Double.compare(oldItem.getPorcentagemOcupada(), newItem.getPorcentagemOcupada()) == 0;
        }
    }
}
